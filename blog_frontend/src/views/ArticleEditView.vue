            ...
            <el-card class="article-create-card">
                ...
                <el-form ref="articleFormRef" :model="articleForm" :rules="articleRules" label-width="100px"
                    ...
                    <el-form-item label="封面图片">
                        ...
                        <!-- 移除封面按钮 -->
                        <el-button v-if="articleForm.cover_image_url" type="danger" size="small"
...
const coverImageChanged = ref(false) // 封面图片是否改变

// ============== 防抖验证字段 ==============
const debouncedValidateField = debounce((prop: string) => {
    articleFormRef.value?.validateField(prop) // 验证指定字段
}, 300)
...
const fetchArticle = async () => {
    ...
    if (articleId) {
        ...
        articleForm.is_published = article.is_published
        articleForm.cover_image_url = article.cover_image || ''
    }
}
...
const handleSubmit = async () => {
    try {
        ...
        } else if (isEditMode.value && coverImageChanged.value) {
            formData.append('cover_image', '') // 表示移除封面
        }

        // 4. 根据模式调用不同API
        let response
        if (isEditMode.value) {
            response = await articleStore.updateArticle(Number(route.params.id), formData)
        } else {
            response = await articleStore.createArticle(formData)
        }
...
const handleCoverChange = (file: UploadFile) => {
    ...
    // 4. 处理有效文件
    if (file.raw) {
        coverImageFile.value = file.raw // 保存文件对象
        articleForm.cover_image_url = URL.createObjectURL(file.raw) // 生成预览URL
    } else {
        coverImageFile.value = null // 清除文件
        articleForm.cover_image_url = '' // 清除预览URL
    }
    coverImageChanged.value = true // 标记封面已更改
    return false
}
...
const handleCancel = () => {
    if (isEditMode.value) {
        router.push(`/articles/${route.params.id}`) // 编辑模式返回详情页
    } else {
        router.push('/articles') // 创建模式返回列表页
    }
}
